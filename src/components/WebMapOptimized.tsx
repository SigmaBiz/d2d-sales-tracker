...
const WebMapOptimized = React.forwardRef<WebView, WebMapProps>(({ knocks, userLocation, onKnockClick, onKnockClear, hailContours = null, activeStorms = [], verifiedReports = [] }, ref) => {
  const webViewRef = useRef<WebView>(null);
  const [isLoading, setIsLoading] = useState(true);

  console.log('WebMapOptimized render - hailContours prop:', hailContours);

  // Expose WebView methods to parent
  React.useImperativeHandle(ref, () => webViewRef.current as WebView);

  // OPTIMIZATION: Generate HTML once with clustering support
  const mapHTML = React.useMemo(() => `
    ...
    <head>
      ...
      <style>
        ...
        /* Custom cluster styles to show outcome distribution */
    ...
    <body>
      <div id="map"></div>
      <script>
        // PRESERVATION: All knock outcome types and emojis (from CORE_ARCHITECTURE_SNAPSHOT.md)
        var colors = {
          not_home: '#6b7280',
          convo: '#3b82f6',
          inspected: '#3b82f6', 
        ...
        // OPTIMIZATION: Reuse console override
        ...
        setTimeout(function() {
          ...
          if (typeof L !== 'undefined') {
            ...
            try {
              ...
              // Map layers (PRESERVED from original)
              ...
              // PRESERVATION: Toggle map type functionality
              ...
              // OPTIMIZATION: Use marker clustering
              ...
              var verifiedMarkers = [];
              var knockMarkerMap = new Map(); // OPTIMIZATION: Track markers by ID
              
              // OPTIMIZATION: Batch marker updates
              window.updateKnocks = function(knocksData) {
                console.log('Updating ' + knocksData.length + ' knocks with clustering');
                
                // OPTIMIZATION: Only update changed markers
                var currentKnockIds = new Set(knocksData.map(function(k) { return k.id; }));
                
                // Remove markers for deleted knocks
                ...
                knocksData.forEach(function(knock) {
                  ...
                  // Check if we need to update an existing marker
                  if (existingMarker) {
                    // Remove old marker to recreate with new data
                    markerClusterGroup.removeLayer(existingMarker);
                    knockMarkerMap.delete(knock.id);
                  }
                  
                  // Create new or updated marker
                  // PRESERVATION: All visual styling and popup content
                  ...
                  // PRESERVATION: Complete popup content with history and all features
                  ...
                  // PRESERVATION: Edit and Clear buttons
              ...
              // PRESERVATION: All other functions remain identical
              ...
              // PRESERVATION: Edit and Clear knock functions
              ...
              // PRESERVATION: Verified reports functionality
              ...
              // PRESERVATION: Hail contours functionality (unchanged)
              ...
              // PRESERVATION: Map click handler
              ...
              // PRESERVATION: Message listener (unchanged)
  ...
  // OPTIMIZATION: Memoize update effects
  useEffect(() => {
    console.log('WebMapOptimized knocks useEffect - isLoading:', isLoading, 'knocks count:', knocks.length);
  ...
  useEffect(() => {
    console.log('WebMapOptimized hailContours useEffect - isLoading:', isLoading, 'webViewRef.current:', !!webViewRef.current, 'hailContours:', hailContours);
    if (!isLoading && webViewRef.current) {
  ...
  useEffect(() => {
    console.log('WebMapOptimized verifiedReports useEffect - isLoading:', isLoading, 'verifiedReports count:', verifiedReports.length);
  ...
  const handleMessage = (event: any) => {
    try {
      ...
      if (data.type === 'console') {
        ...
        console.log('WebMapOptimized received mapReady message');
        setIsLoading(false);
      } else if (data.type === 'mapError') {
        console.error('WebMapOptimized error:', data.error);
        setIsLoading(false);
      ...
      console.error('WebMapOptimized message error:', error);
    }
  };
...
export default WebMapOptimized;
