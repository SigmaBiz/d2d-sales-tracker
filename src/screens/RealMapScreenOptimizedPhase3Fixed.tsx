...
export default function RealMapScreenOptimized({ navigation }: any) {
  ...
  const updateUserLocationDebounced = useMemo(
  ...
  // PRESERVE ORIGINAL FUNCTIONALITY: updateLocation function
  ...
  // PRESERVE ORIGINAL FUNCTIONALITY: centerOnUser exactly as it was
  const centerOnUser = () => {
    updateLocation();
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        // Get location first - PRESERVE ORIGINAL BEHAVIOR
        await getLocation();
  ...
  // PRESERVE ORIGINAL FUNCTIONALITY: getLocation function
      ...
      if (hasPermission) {
        ...
        subscription = await LocationService.watchPosition(
          (location) => {
            ...
            updateUserLocationDebounced(location); // This is now debounced
          },
  ...
  }, [isFocused, updateUserLocationDebounced]);
